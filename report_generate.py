...
def ReportGenerate(patient_data):
    ...
    y = height - 30  # Initialize y-coordinate for drawing
    ...
    y = height - 80  # Reset y-coordinate after header

    def draw_patient_info(y):
        """ Draws patient information. """
        c.setFont("Helvetica", 12)
        info_lines = [
    ...
    def draw_tests_results(y):
        ...
        # Create the table
        table = Table(data, colWidths=[80, 100, 80, 120])  # Adjust column widths as needed
        ...
        c.setFont("Helvetica-Bold", 14)
        heading_width = c.stringWidth(heading, "Helvetica-Bold", 14)
        c.drawString((width - heading_width) / 2, y, heading)  # Center the heading
        y -= 20  # Move down after the heading

        # Wrap the table to fit
        table.wrapOn(c, width - 120, height)
        check_space_needed(150)  # Check for space before drawing the table
        table.drawOn(c, 60, y - 150)  # Position the table directly under the heading

    y = draw_bordered_section("Test Results", y, draw_tests_results)

    def draw_prescription(y):
        """ Draws a medical prescription section. """
        c.setFont("Helvetica", 12)
