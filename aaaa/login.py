...
class Login(ctk.CTkFrame):
    def __init__(self, master, db, on_login_success, on_switch_to_signup):
        ...
        self.configure(fg_color="#E5E5E5")  # Light gray background like in the image
        self.pack(fill="both", expand=True)

        self.create_login_form()

    def create_login_form(self):
        # Create the centered black card
        login_card = ctk.CTkFrame(
            self,
            fg_color="#2B2B2B",
            corner_radius=15,
            width=340,  # Reduced width to match image
            height=380  # Reduced height to match image
        )
        login_card.place(relx=0.5, rely=0.5, anchor="center")
        login_card.pack_propagate(False)  # Prevent automatic resizing
        ...
        title.pack(pady=(40, 20))  # Adjusted padding
    ...
    def set_callbacks(self, login_callback=None, signup_callback=None):
        """Set callbacks for login and signup events"""
        if login_callback:
            self.login_callback = login_callback
        if signup_callback:
            self.switch_to_signup_callback = signup_callback
